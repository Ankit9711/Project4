...
for i in range(1,exp+1) :
    ...
    Exp_categ=input(f"Expenses{i} Category: ").upper()
    print("\n\n")
    d_CategwithExp[Exp_categ]=d_CategwithExp.get(Exp_categ,0)+Exp_am
print(f"{status}\n")
for i in d_CategwithExp.values() :
    L_Exp.append(i)
Total=0
for item  in L_Exp :
    Total+=item
